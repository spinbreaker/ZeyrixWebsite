...
import { Message, Attachment, AgentStep, ApprovalDetails } from "@/src/types/chat";
...
function UserMessage({ attachments, text, time }: { attachments: Attachment[] | undefined, text: string, time: string }) {
    return (
...
function ApprovalCard({ approvalDetails, applyToolUse }: { approvalDetails: ApprovalDetails | undefined, applyToolUse: (requestId: string, action: "confirm" | "reject") => Promise<void> }) {
...
function Step({ kind, label, status, toolName, isLast }: AgentStep) {
  let stepAction = "Unknown: ";
  if (kind === "reasoning") {
    stepAction = "Reasoning: ";
  } else if (kind === "tool_call") {
    stepAction = `Tool «${toolName}»: `;
  }
  ...
  return (
    <div className={`flex gap-2 items-start ${colorClass}`}>
      <div className="relative flex w-3 shrink-0 flex-col items-center self-stretch">
        ...
        {!isLast && (
          <div className="w-0.5 flex-1 -mb-1 bg-foreground-muted" />
        )}
      </div>
...
export function MessageBubble({ applyToolUse, role, status, text, attachments, steps, createdAt, processingSeconds }: Message) {
    const t = useTranslations("agentSteps");

    const time = formatMessageTime(createdAt);
    const [isStepsExtended, setIsStepsExtended] = useState(false);
    ...
    const workDuration = formatDuration(useElapsedTime(createdAt) + (processingSeconds ? processingSeconds * 1000 : 0));
    const workedFor = processingSeconds ? formatDuration(processingSeconds * 1000) : "-";
    ...
    //         id: "step-003",
    //         kind: "reasoning",
    //         label: "Preparing the final response",
    //         status: "in_progress",
    //         approvalDetails: undefined,
    //     },
    // ];
        ...
        case "assistant":
            return (
                <div className="flex justify-start min-w-0">
                    <div className="py-3 rounded-t-[20px] rounded-br-[20px] rounded-bl-sm w-full flex flex-col gap-2 min-w-0">
                        <div className="flex flex-col gap-2">
                            ...
                            >
                                {status === "pending" ? (
                                    <p className="text-body-sm">{t("working")} {workDuration}</p>
                                ) : status === "approval_required" ? (
                                    <p className="text-body-sm">{t("awaiting")}</p>
                                ) : (
                                    <p className="text-body-sm">{t("worked")} {workedFor}</p>
                                )}
                                <ArrowIcon className={`size-2 ${isStepsExtended && "rotate-90"}`} />
                            </div>
                        ...
                        {status === "approval_required" && (
                            <ApprovalCard approvalDetails={approvalDetails} applyToolUse={applyToolUse} />
                        )}
                    </div>
                </div>
            );
